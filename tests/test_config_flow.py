...
from custom_components.the511.const import DOMAIN


async def test_user_step_shows_form(hass):
    """The initial step should render the user form."""
...
async def test_user_step_creates_entry(hass):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], {CONF_NAME: "Wisconsin"}
    )
...
async def test_config_flow_unique_id(hass):
