"""Tests for the New Brunswick 511 provider."""
...
from custom_components.the511.providers.new_brunswick import NewBrunswickProvider
...
def _url(resource: str, version: int = 2) -> str:
    ...
    return f"{NewBrunswickProvider.base_url}/api/v{version}/get/{resource}"


def _provider(hass):
    """Build a provider bound to the mocked aiohttp session."""
    return NewBrunswickProvider(
        session=async_get_clientsession(hass),
        config={CONF_API_KEY: API_KEY},
    )


def test_new_brunswick_provider_registered():
    """New Brunswick should be registered under its provider_id."""
    assert get_provider_class("new_brunswick") is NewBrunswickProvider


def test_new_brunswick_capabilities():
    """New Brunswick supports cameras, incidents, and road conditions."""
    provider = NewBrunswickProvider(session=None, config={CONF_API_KEY: API_KEY})
...
async def test_road_conditions_use_primary_conditions(hass, aioclient_mock):
    """New Brunswick winter roads report the surface under ``Primary Conditions``."""
    aioclient_mock.get(
        _url("winterroads", version=3),
        json=[{"RoadwayName": "NB-2", "Primary Conditions": "Bare Dry"}],
    )

    conditions = await _provider(hass).async_get_road_conditions()

    assert len(conditions) == 1
    assert conditions[0].road == "NB-2"
