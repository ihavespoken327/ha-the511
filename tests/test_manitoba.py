"""Tests for the Manitoba 511 provider."""
...
from custom_components.the511.providers.manitoba import ManitobaProvider
...
def _url(resource: str, version: int = 2) -> str:
    ...
    return f"{ManitobaProvider.base_url}/api/v{version}/get/{resource}"


def _provider(hass):
    """Build a provider bound to the mocked aiohttp session."""
    return ManitobaProvider(
        session=async_get_clientsession(hass),
        config={CONF_API_KEY: API_KEY},
    )


def test_manitoba_provider_registered():
    """Manitoba should be registered under its provider_id."""
    assert get_provider_class("manitoba") is ManitobaProvider


def test_manitoba_capabilities():
    """Manitoba supports cameras, incidents, and road conditions."""
    provider = ManitobaProvider(session=None, config={CONF_API_KEY: API_KEY})
...
async def test_road_conditions_use_primary_condition(hass, aioclient_mock):
    """Manitoba winter roads report the surface under ``Primary Condition``."""
    aioclient_mock.get(
        _url("winterroads", version=3),
        json=[{"RoadwayName": "MB-1", "Primary Condition": "Ice Covered"}],
    )

    conditions = await _provider(hass).async_get_road_conditions()

    assert len(conditions) == 1
    assert conditions[0].road == "MB-1"
    assert conditions[0].surface == "Ice Covered"
