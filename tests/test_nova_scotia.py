"""Tests for the Nova Scotia 511 provider."""
...
from custom_components.the511.providers.nova_scotia import NovaScotiaProvider
...
def _url(resource: str, version: int = 2) -> str:
    ...
    return f"{NovaScotiaProvider.base_url}/api/v{version}/get/{resource}"


def _provider(hass):
    """Build a provider bound to the mocked aiohttp session."""
    return NovaScotiaProvider(
        session=async_get_clientsession(hass),
        config={CONF_API_KEY: API_KEY},
    )


def test_nova_scotia_provider_registered():
    """Nova Scotia should be registered under its provider_id."""
    assert get_provider_class("nova_scotia") is NovaScotiaProvider


def test_nova_scotia_capabilities():
    """Nova Scotia is limited to cameras and incidents (no public docs)."""
    provider = NovaScotiaProvider(session=None, config={CONF_API_KEY: API_KEY})
