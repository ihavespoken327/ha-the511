"""Tests for the Pennsylvania 511 provider."""
...
from custom_components.the511.providers.pennsylvania import PennsylvaniaProvider
...
def _url(resource: str, version: int = 2) -> str:
    ...
    return f"{PennsylvaniaProvider.base_url}/api/v{version}/get/{resource}"


def _provider(hass):
    """Build a provider bound to the mocked aiohttp session."""
    return PennsylvaniaProvider(
        session=async_get_clientsession(hass),
        config={CONF_API_KEY: API_KEY},
    )


def test_pennsylvania_provider_registered():
    """Pennsylvania should be registered under its provider_id."""
    assert get_provider_class("pennsylvania") is PennsylvaniaProvider


def test_pennsylvania_capabilities():
    """Pennsylvania is limited to cameras and incidents (no public docs)."""
    provider = PennsylvaniaProvider(session=None, config={CONF_API_KEY: API_KEY})
